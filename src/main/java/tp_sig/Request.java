...
public class Request {
    ...
    public static void getGrenobleBuilding(Connection connection) throws SQLException {
        ...
        //prepareStatement("SELECT linestring, tags->'highway' as highway FROM ways WHERE tags?'highway' LIMIT 1;");
        //result limited to 3 right now
        res = st.executeQuery("SELECT linestring  FROM ways WHERE tags?'building' AND ST_Intersects(ways.bbox,ST_SetSRID(ST_MakeBox2D(ST_Point(5.7,45.1),ST_Point(5.8,45.2)),4326));");
        ...
        while (res.next()) {
            ...
            for (int i = 0; i < g.numPoints(); i++){
                ...
                //Print all the points
                System.out.println("Nom = " + res.getString("Building"));
                System.out.println("\t\tLongitude = " + p.getX());
                System.out.println("\t\tLatitude = " + p.getY());
            }
    ...
    public static Color getColorAlea(){
    ...
    public static void getGrenobleSchool(Connection connection) throws SQLException {
        ...
        while (res.next()) {
            ...
            Point p = null;
            geoexplorer.gui.Point drawedPoint = null;
            geoexplorer.gui.Polygon drawedPolygon = new geoexplorer.gui.Polygon(Color.blue,Color.white);
            ...
            for (int i = 0; i < g.numPoints(); i++){
                ...
                drawedPolygon.addPoint(drawedPoint);
                //Print all the points
                System.out.println("Nom = " + res.getString("quartier"));
                System.out.println("\t\tLongitude = " + p.getX());
                System.out.println("\t\tLatitude = " + p.getY());
           ...
           /* String requetebis = " SELECT  linestring  " +
                    "FROM ways, quartier WHERE ways.tags->'amenity' like '%school%'" + "AND quartier.quartier like ? " ;

            ResultSet result  ;
            PreparedStatement state = connection.prepareStatement(requetebis);
            state.setString(1,res.getString("quartier"));
            result = state.executeQuery();
            Boolean continueu = true ;
            while (result.next() && continueu) {
                for (int i = 0; i < g.numPoints(); i++) {
                    p = g.getPoint(i);
                    drawedPoint = new geoexplorer.gui.Point(p.getX(), p.getY(), Color.blue);
                    drawedPolygon.addPoint(drawedPoint);
    ...
    public static void getGrenobleSchoolPerQuartier(Connection connection) throws SQLException {

        MapPanel  map = new MapPanel(4.75,44.01,0.1);
        GeoMainFrame geo = new GeoMainFrame("Map", map);

        st = connection.createStatement();
        String requete = " SELECT  ways.linestring   " +
                "FROM ways, quartier WHERE ways.tags->'amenity' like '%school%' AND quartier.quartier like 'NOTRE DAME' " +
                " and ST_Intersects(ST_Transform(quartier.the_geom,2154)::geometry, ways.linestring) ";
